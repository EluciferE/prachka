...
class Token:
    ...
    def __init__(self, username: str,
        ...
        self.tokens_logger = logging.getLogger('token')
        self.tokens_logger.addHandler(tokens_logs)
        self.tokens_logger.propagate = False
    ...
    def refresh_token(self):
        ...
        if "error" not in r.text:
            self.tokens_logger.info(f"Success refresh token by {self._username}")
            token_json = r.json()

            if "refresh_token" not in token_json:
                token_json["refresh_token"] = self._token["refresh_token"]
            else:
                self.tokens_logger.info(f"Got new refresh token for {self._username}")
            ...
            self.tokens_logger.error(f"Refresh token by {self._username}: {r.text}")
            return -1
    ...
    def validate_token(self):
        ...
        r = requests.get(url)
        if r.json().get("error"):
            self.tokens_logger.info(f"Bad validate of token by {self._username}")
            return False
        return True
